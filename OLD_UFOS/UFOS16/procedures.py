        ...
        if chan == 'ZD':
            ...
            text_out = ';'.join([str(i) for i in
                                 [date_utc, date_local, sunheight,
                                  ...
                                  calc_result['o3_2'], *calc_result['additional_data_2'], calc_result['correct_2']]]).replace(
                                      ".", ",")
            dirs = ['Ufos_{}'.format(pars['device']['id']),
                    type_of_measurement,
                    datetime.datetime.strftime(date, '%Y'),
...
class Main:
    ...
    def add_calculated_line_to_final_file(self, pars, home, spectr, mu, expo, sensitivity, sensitivity_eritem,
        ...
        if self.chan == 'ZD':
            ...
            for pair in ["1", "2"]:
                for text, value in zip(["o3", "correct", "additional_data"], [int(o3[pair]), correct[pair], additional_data[pair]]):
