...
def plot_uv2(o3_mode, ImageTk, ttk, some_root, plt, rc, canvas, home, plotx, ploty, xmax, ymax, x, y):
    try:
        new_x = []
        for i in x:
            if i.count(':') > 0:
        ...
        if o3_mode in ['uva', 'uvb', 'uve']:  # uva,uvb,uve
            ...
            shag_y = 10
        ma_y = []
        for i in range(zero, max_y, shag_y):
            ma_y.append(i)
        ax.set_yticks(ma_y)
        ax.set_ylim(zero, max_y)
    ...
    try:
        ...
        canvas.create_image(xmax // 2, ymax, anchor='s', image=photoimage)
    except Exception as err:
        print(err)
    return (0)
