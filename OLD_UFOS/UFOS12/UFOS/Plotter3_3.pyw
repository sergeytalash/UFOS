...
if sys_pf == 'darwin':
    ...
    from OLD_UFOS.UFOS12.UFOS.Shared_ import *
else:
    from Shared_ import *
...
def graph_in(o3_mode, gr_ok, t, kx, ky, x, y, plotx, ploty, xshag, yshag, some_root):
    ...
    ####                    else:
    ####                        text_time = str(minutes//60+curr_time[0]) + ':30'
    ...
    ##            yline = ploty + my
    ##            while yline>=my:
    ##                ima.append(canvas.create_line((mx, ploty+2*my-yline, plotx+mx, ploty+2*my-yline), fill='grey'))
    ##                ima.append(canvas.create_text(mx-15, ploty+2*my-yline, text=str(round((yline-my)/ky+o3min)), fill='black'))
    ##                yline-=yshag * 2
    ...
    if t != 0 and gr_ok != 0:
        if o3_mode == 'spectr':
            ...
            uvalab = plot_uv2(o3_mode, ImageTk, ttk, some_root, plt, rc, canvas, home, plotx, ploty, xmax, ymax,
                              t['time'], t['data'])
        ##            lab_ozon.configure(text='Значение УФ-А: {0}'.format(str(uv)))
        elif o3_mode == 'uvb':
            ##            uvblab = plot_uv(v,t,curr_time,canvas,plotx,ploty,kx,ky,mx,my,o3_mode)
            uvalab = plot_uv2(o3_mode, ImageTk, ttk, some_root, plt, rc, canvas, home, plotx, ploty, xmax, ymax,
                              t['time'], t['data'])
        ##            lab_ozon.configure(text='Значение УФ-Б: {0}'.format(str(uv)))
        elif o3_mode == 'uve':
            ##            uvelab = plot_uv(v,t,curr_time,canvas,plotx,ploty,kx,ky,mx,my,o3_mode)
            uvalab = plot_uv2(o3_mode, ImageTk, ttk, some_root, plt, rc, canvas, home, plotx, ploty, xmax, ymax,
                              t['time'], t['data'])
        ##            lab_ozon.configure(text='Значение УФ-Э: {0}'.format(str(uv)))
        elif o3_mode == 'ozone':
            """График озона"""
            r = 3
            ozon_x = []
            ...
            while ji < len(t['data']):
                if t['color'][ji] == 'black':
                    ozon_x.append(t['time'][ji])
                    ozon_y.append(t['data'][ji])
                ji += 1
            if v == 0:
                ...
                ##                    try:
                ##                        color = t['color'][i]
                ##                        point1x = round(((int(t['time'][i][:2])+int(t['time'][i][3:5])/60)-min(curr_time))/ppp*plotx)
                ##                        point1y = round(t['data'][i] * ky - o3min * ky)
                ##                        if point1y>0:
                ##                            ozon_x.append(point1x)
                ##                        point2x = round(((int(t['time'][i+1][:2])+int(t['time'][i+1][3:5])/60)-min(curr_time))/ppp*plotx)
                ##                        point2y = round(t['data'][i+1] * ky - o3min * ky)
                ##                        ozon_x.append(point2x)
                ##                        ozon_y.append(point2y)
                ##                        if color== 'black':
                ##                            ima.append(canvas.create_oval(point1x + mx - r, ploty - point1y + my - r, point1x + mx + r, ploty - point1y + my + r,outline=color,fill=color))
                ##                    except:
                ##                        pass
                uvalab = plot_uv2(o3_mode, ImageTk, ttk, some_root, plt, rc, canvas, home, plotx, ploty, xmax, ymax,
                                  ozon_x, ozon_y)
