...
##    global main.next_start
    ...
    count = 0
    flag = True
    path = os.path.join(home,y,m,d)
##    print([main.next_start])
    next_st = datetime.datetime.strptime(main.next_start,'%Y-%m-%d %H:%M')
##    print(next_st,datetime.datetime.now())   
    ...
    if d:
        if k == 1 or chan == 'D':
            try:
                ...
                #tmp_path
                tmp_path = try_join_path(tmp_path,date_mas[2])
                tmp_path = try_join_path(tmp_path,date_mas[1])
                tmp_path = try_join_path(tmp_path,date_mas[0])
                #sens_tmp_path
                ...
                file_o = open(os.path.join(tmp_path,name),'w')
                file_data = """;UFOS3
